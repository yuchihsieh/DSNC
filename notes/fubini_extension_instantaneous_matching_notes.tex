...
We now construct an \emph{instantaneous} random full matching with strong measure-preserving properties and the independence-in-types feature used in applications. This follows the Loeb-space proof scheme used in Duffie--Sun (2007, Section 4.1) for Theorem 2.4.
...
\]
The crucial property established in Duffie--Sun (2007, Theorem 2.4) is that $g$ can be constructed so as to be \emph{essentially pairwise independent}: for $\lambda$-a.e.\ $i$, the random variables $g_i$ and $g_j$ are independent for $\lambda$-a.e.\ $j$. This is the exact independence notion that is compatible with continuum indexing and sufficient for exact law of large numbers arguments.
...
Given such a uniform family, one can generate essentially pairwise independent families with \emph{any prescribed distributions} by measurable transforms (inverse CDF method), provided the target distributions are Borel on a Polish space. This is the practical meaning of ``rich enough.''
\end{remark}
